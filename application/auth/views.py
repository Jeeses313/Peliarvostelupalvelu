...
def auth_profile(user_id):
    ...
    stmt = text("SELECT Review.id, Game.name, Account.username, Review.grade, Review.text, COUNT(Like.id) AS like_count, SUM(CASE Like.user_id WHEN 2 THEN 1 ELSE 0 END) AS is_liked, Account.id, Game.id FROM Review LEFT JOIN Game ON Review.game_id = Game.id LEFT JOIN Account ON Review.user_id = Account.id LEFT JOIN Like ON Review.id = Like.review_id WHERE Account.id = :user_id GROUP BY Review.id, Game.name, Account.username, Account.id, Game.id").params(user_id=user_id)
    res = db.engine.execute(stmt)
    reviews = []
