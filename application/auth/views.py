...
def auth_profile(user_id):
    ...
    stmt = text("SELECT Review.id, Game.name, Account.username, Review.grade, Review.text, Account.id, Game.id FROM Review LEFT JOIN Game ON Review.game_id = Game.id LEFT JOIN Account ON Review.user_id = Account.id WHERE Account.id = :user_id GROUP BY Review.id, Game.name").params(user_id=user_id)
    res = db.engine.execute(stmt)
    reviews = []
