...
def auth_profile(user_id):
    ...
    res = db.engine.execute("SELECT User.id, User.username, COUNT(Review.id) AS review_count FROM User LEFT JOIN Review ON User.id = Review.user_id WHERE User.id= ? GROUP BY User.id", user_id)
    users = []
    for row in res:
